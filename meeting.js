...
joinBtn.addEventListener('click', () => {
    // Sanitize ID: Remove spaces, special chars that might come from bad copy-paste
    const rawId = meetingIdInput.value;
    const id = rawId.replace(/[^a-zA-Z0-9-_]/g, '').trim();

    if (id) {
        meetingIdInput.value = id; // Update UI with clean ID
        startMeeting(false, id);
    }
    else showToast('Invalid Meeting ID', 'error');
});
...
function initPeer(customId) {
    // Default PeerJS options + Custom ICE Servers
    const peerOptions = {
        config: iceConfig,
        debug: 2
    };
    ...
    peer.on('error', (err) => {
        ...
        if (err.type === 'unavailable-id') {
            ...
            showToast('Host not found. Retrying in 2s...', 'warning');
            setTimeout(() => {
                if (!isHost && hostId) connectToHost(hostId);
            }, 2000);
        }
    });
...
function updateLayout(pid) {
    presenterId = pid;
    const grid = document.getElementById('video-grid');

    if (presenterId) {
        grid.classList.add('presentation-mode');
        // Mark the presenter tile
        const tiles = document.querySelectorAll('.video-tile');
        tiles.forEach(tile => {
            tile.classList.remove('presenter');
            // Check if this tile belongs to presenter
            // My ID check or Peer ID check
            if (pid === myPeerId && tile.classList.contains('is-local')) {
                tile.classList.add('presenter');
            } else if (tile.id === 'video-' + pid) {
                tile.classList.add('presenter');
            }
        });
    } else {
        grid.classList.remove('presentation-mode');
        document.querySelectorAll('.video-tile').forEach(t => t.classList.remove('presenter'));
    }
}
...
function addVideoStream(stream, name, isLocal, peerId) {
    ...
    // Inject Remove Button if Host
    if (isHost && !isLocal && peerId) {
        const btnKick = document.createElement('button');
        btnKick.className = 'kick-btn';
        btnKick.innerHTML = '<i class="ph ph-user-minus"></i>';
        btnKick.title = 'Remove User';
        btnKick.onclick = (e) => {
...
function kickUser(peerId) {
    ...
    // Send Kick Signal
    if (peers[peerId].conn && peers[peerId].conn.open) {
        peers[peerId].conn.send({ type: 'kick-user' });
    }
...
async function toggleScreenShare() {
    ...
    try {
        ...
        // Broadcast Start
        broadcastData({ type: 'screen-share-started', peerId: myPeerId });
        updateLayout(myPeerId);
...
function stopScreenShare() {
    ...
    // Broadcast Stop
    broadcastData({ type: 'screen-share-stopped' });
    updateLayout(null);
}
...
handleData = function (data) {
    if (data.type === 'chat') {
        ...
        setTimeout(() => {
            window.location.href = window.location.pathname; // Reload/Exit
        }, 2000);
        if (peer) peer.destroy();
    }
    else if (data.type === 'screen-share-started') {
        updateLayout(data.peerId);
